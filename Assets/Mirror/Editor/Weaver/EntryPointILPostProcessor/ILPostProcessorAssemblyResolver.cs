    ...
    class ILPostProcessorAssemblyResolver : IAssemblyResolver
    {
        ...
        public AssemblyDefinition Resolve(AssemblyNameReference name, ReaderParameters parameters)
        {
            ...
            if (fileName == null)
            {
                ...
                // Newtonsoft.Json lives in PackageCache only (not ScriptAssemblies), so the
                // parent-directory search never finds it — but Mirror weaver doesn't need it.
                if (!name.Name.StartsWith("Bee.BeeDriver") && name.Name != "Newtonsoft.Json")
                {
