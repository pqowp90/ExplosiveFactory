...
namespace Mirror.FizzySteam
{
    public abstract class LegacyCommon
    {
        ...
        private void OnConnectFail(SteamId id, P2PSessionError err)
        {
            ...
            throw err switch
            {
                P2PSessionError.NoRightsToApp => new Exception(
                    "Connection failed: The local user doesn't own the app that is running."),
                P2PSessionError.Timeout => new Exception(
                    "Connection failed: The connection timed out because the target user didn't respond."),
                _ => new Exception("Connection failed: Unknown error.")
            };
        }
