...
namespace Mirror.FizzySteam
{
    ...
    public class FizzyFacepunch : Transport
    {
        ...
        [SerializeField] public P2PSend[] Channels = new P2PSend[2] { P2PSend.Reliable, P2PSend.UnreliableNoDelay };
        ...
        [Header("Info")] [Tooltip("This will display your Steam User ID when you start or connect to a server.")]
        ...
        public override void ClientConnect(string address)
        {
            ...
            {
                Debug.LogWarning("[FizzyFacepunch] Cannot connect to yourself.");
                OnClientDisconnected.Invoke();
                return;
            }
            ...
            if (!ClientActive() || client.Error)
            {
                if (UseNextGenSteamNetworking)
                {
                    ...
                    var newClient = NextClient.CreateClient(this, address);
                    if (newClient.Error)
                    {
                ...
                else
                {
                    ...
                    var newClient = LegacyClient.CreateClient(this, address);
                    if (newClient.Error)
                    {
                        Debug.LogError("Failed to start client.");
                        return;
        ...
        public override string ServerGetClientAddress(int connectionId) =>
            ServerActive() ? server.ServerGetClientAddress(connectionId) : string.Empty;
        ...
        private bool InitialiseSteamworks(uint appid)
        {
            ...
            {
                Debug.LogError(
                    $"Could be one of the following: Steam is closed, Can't find steam_api dlls or Don't have permission to open appid. Exception: {e.Message}");
                return false;
            }
