...
namespace Mirror.RemoteCalls
{
    ...
    public static class RemoteProcedureCalls
    {
        ...
        public static string GetMethodNameFromHash(ushort hash)
        {
            static string FixMethodName(string methodName)
            {
                methodName = methodName.Replace("InvokeUserCode_", "");

                var index = methodName.IndexOf("__", StringComparison.Ordinal);
                if (index != -1) methodName = methodName[..index];

                return methodName;
            }

            return
                $"{remoteCallDelegates[hash].componentType}.{FixMethodName(remoteCallDelegates[hash].function.GetMethodName())}";
        }
