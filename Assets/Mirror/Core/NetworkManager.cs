...
#pragma warning restore 618
        ...
        public virtual void OnApplicationQuit()
        {
            ...
            if (NetworkServer.active)
            {
                ...
                {
                    Debug.LogWarning("Error while shutting down server.");
                }
        ...
        public static UniTask? loadingSceneAsync;
        ...
        public virtual void ServerChangeScene(string newSceneName)
        {
            ...
            loadingSceneAsync = SceneManager.LoadSceneAsync(newSceneName).ToUniTask();
            ...
            switch (sceneOperation)
            {
                ...
                case SceneOperation.LoadAdditive:
                    ...
                    if (!SceneManager.GetSceneByName(newSceneName).IsValid() && !SceneManager.GetSceneByPath(newSceneName).IsValid())
                        loadingSceneAsync = SceneManager.LoadSceneAsync(newSceneName, LoadSceneMode.Additive).ToUniTask();
                    else
                    {
                ...
                case SceneOperation.UnloadAdditive:
                    ...
                    // since we don't know which was passed in the Scene message
                    if (SceneManager.GetSceneByName(newSceneName).IsValid() || SceneManager.GetSceneByPath(newSceneName).IsValid())
                        loadingSceneAsync = SceneManager.UnloadSceneAsync(newSceneName, UnloadSceneOptions.UnloadAllEmbeddedSceneObjects).ToUniTask();
                    else
                    {
        ...
        void UpdateScene()
        {
            if (loadingSceneAsync != null && loadingSceneAsync.Value.Status == UniTaskStatus.Succeeded)
            {
        ...
        void OnServerAddPlayerInternal(NetworkConnectionToClient conn, AddPlayerMessage msg)
        {
            ...
            {
                // 씬 전환 등으로 이미 플레이어가 존재하는 경우 안전하게 리턴
                return;
            }
        ...
        /// <summary>Called from ServerChangeScene immediately before SceneLoader.LoadSceneAsync is executed</summary>
        ...
        /// <summary>Called from ClientChangeScene immediately before SceneLoader.LoadSceneAsync is executed</summary>
