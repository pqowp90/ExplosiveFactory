...
namespace Mirror
{
    ...
    public abstract class NetworkBehaviour : MonoBehaviour
    {
        ...
        protected virtual void OnValidate()
        {
            ...
            {
                // warning if not in prefab or prefab stage
            ...
            int channelId, bool requiresAuthority = true)
        {
            ...
            if (!NetworkClient.active)
            {
                Debug.LogWarning($"Command {functionFullName} called on {name} without an active client.", gameObject);
                return;
            }
            ...
            int channelId, bool includeOwner)
        {
            if (this == null)
            {
                Debug.LogError($"RPC Function {functionFullName} called on a destroyed object.");
                return;
            }
