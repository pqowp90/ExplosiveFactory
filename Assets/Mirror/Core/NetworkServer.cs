        ...
        static void OnCommandMessage(NetworkConnectionToClient conn, CommandMessage msg, int channelId)
        {
            if (!conn.isReady)
            {
                ...
                if (channelId == Channels.Reliable)
                {
                    ...
                    RemoteProcedureCalls.GetFunctionMethodName(msg.functionHash, out string notReadyMethodName);
                    Debug.LogWarning($"Command {notReadyMethodName ?? $"hash={msg.functionHash}"} received while client is not ready.\nThis may be ignored if client intentionally set NotReady.");
                }
                return;
            }
            ...
            if (!spawned.TryGetValue(msg.netId, out NetworkIdentity identity))
            {
                ...
                if (channelId == Channels.Reliable)
                    Debug.LogWarning($"Spawned object not found when handling Command message netId={msg.netId}\nIn handling Command <color=Orange>[{RemoteProcedureCalls.GetMethodNameFromHash(msg.functionHash)}]</color>");
                return;
            }
