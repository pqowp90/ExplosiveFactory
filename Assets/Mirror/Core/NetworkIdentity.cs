...
namespace Mirror
{
    ...
    public sealed class NetworkIdentity : MonoBehaviour
    {
...
#if UNITY_EDITOR
            // Project-Eddy patch: 최신 Unity에서는 런타임 Instantiate 중에도
            // OnValidate가 호출되며, SetupIDs 내부 PrefabUtility 호출이 자식
            // SpriteRenderer의 OnSpriteRendererBoundsChanged SendMessage를
            // 촉발해 "SendMessage cannot be called during ... OnValidate"
            // 경고를 낸다. 런타임 인스턴스는 이미 ID가 세팅되어 있으므로
            // Play 모드에서는 에디터 전용 로직을 건너뛴다.
        ...
        // fired once, right after all NetworkBehaviour.OnStartClient() calls finish.
        public event Action? AfterStartClient;
